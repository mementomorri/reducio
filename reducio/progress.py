...
class _Progress:
    ...
    def update(self, message: str) -> None:
        with self.lock:
            ...
            self.since = monotonic()
            print(f"[reducio] {message}", file=self.stream, flush=True)
        ...
        while not self.stopped.wait(self.interval):
            with self.lock:
                ...
                if elapsed >= self.interval:
                    print(
                        f"[reducio] Still working: {self.message} ({elapsed:.0f}s elapsed)",
                        file=self.stream,
                        flush=True,
                    )


_active: ContextVar[_Progress | None] = ContextVar("reducio_progress", default=None)
...
def progress(message: str, *, quiet: bool = False, interval: float = 5.0) -> Iterator[None]:
    ...
    try:
        if reporter is not None:
            reporter.update(message)
            thread = Thread(target=reporter.heartbeat, name="reducio-progress", daemon=True)
            thread.start()
        yield
    finally:
        if reporter is not None:
